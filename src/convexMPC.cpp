...
convexMPC::convexMPC(int horizonLength, double dt)
...
void convexMPC::setInitialState(const VectorXd &initialState) {
    x = initialState;
}

void convexMPC::updateDynamicsMatrices(const MatrixXd &A_new, const MatrixXd &B_new) {
    A = A_new;
    B = B_new;
}

void convexMPC::computeMPC(const VectorXd &x_ref, const VectorXd & /* u_ref */) {
